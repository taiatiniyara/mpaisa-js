...
**Client Package**:
The browser-safe subset of mpaisa-js, imported from `"mpaisa-js/client"`. Exports `parseRedirect`, `ResponseCode` constants, and a lightweight `redirectOutcome` — all pure functions with no secrets. Runs in any JavaScript frontend (React, React Native, Vue, Svelte, plain JS); only requires the standard `URL`/`URLSearchParams` globals. The Integrator's frontend uses it for preliminary redirect interpretation; authoritative Confirmation remains server-side. "Client" survives only in this fixed name and **Client ID** — never for a person (that is the Integrator). _Avoid_: client SDK, frontend SDK
